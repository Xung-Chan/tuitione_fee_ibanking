...
class HistoryVM @Inject constructor(
    ...
    private val _uiState = MutableStateFlow(HistoryUS())
    val uiState: StateFlow<HistoryUS> = _uiState.asStateFlow()

    private val _uiEffect = MutableSharedFlow<HistoryEffect>()
    val uiEffect: SharedFlow<HistoryEffect> = _uiEffect.asSharedFlow()
    ...
    fun onEvent(event: HistoryEvent) {
        viewModelScope.launch {
            when (event) {
                is HistoryEvent.SelectPayment -> onSelectedPaymentChanged(event.payment)
            }
        }
    }
    ...
    private fun onSelectedPaymentChanged(payment: Payment) {
        selectedPayment.setPayment(payment)
        viewModelScope.launch {
            _uiEffect.emit(
